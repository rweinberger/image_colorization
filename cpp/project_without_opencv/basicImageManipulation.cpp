...
Image rgb2yuv(const Image &im) {
  ...
  for (int j = 0; j < im.height(); j++)
    for (int i = 0; i < im.width(); i++) {
      output(i, j, 0) =
          0.257 * im(i, j, 0) + 0.504 * im(i, j, 1) + 0.098 * im(i, j, 2) + 16;
      output(i, j, 1) =
          -0.148 * im(i, j, 0) - 0.291 * im(i, j, 1) + 0.439 * im(i, j, 2) + 128;
      output(i, j, 2) =
          0.439 * im(i, j, 0) - 0.368 * im(i, j, 1) - 0.071 * im(i, j, 2) +128;
    }
  return output;
}
...
Image yuv2rgb(const Image &im) {
  ...
  for (int j = 0; j < im.height(); j++)
    for (int i = 0; i < im.width(); i++) {
      float y = im(i,j,0) - 16;
      float u = im(i,j,1) - 128;
      float v = im(i,j,2) - 128;
      output(i, j, 0) = 1.164*y + 1.596 *v;
      output(i, j, 1) = 1.164*y - 0.813*u - 0.391*v;
      output(i, j, 2) = 1.164*y + 2.018*u;
    }
  return output;
}
